...
### 2. Connect-time log reports what the controller is actually running with

When the controller connects, the log now lists both axes' **gear ratio, backlash, backlash mode and speed**, plus the **sky rate that speed implies**. If a motor panel setting silently failed to reach the controller (as happened with driver currents in an earlier rc), this is where you'd see the mismatch — the connect line reports what the controller has, not what the panel shows.
...
4. Everything from the rc10/rc11/rc12 plans still applies (driver currents in the log, STOP, calibration with payload mounted, backlash in the motor panel only, speed hint).
