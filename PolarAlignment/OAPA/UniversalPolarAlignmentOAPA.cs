...
namespace NINA.Plugins.PolarAlignment.OAPA {
    public partial class UniversalPolarAlignmentOAPA : UniversalPolarAlignmentBase {
        ...
        protected override TimeSpan MovementTimeout(float gearRatio, float commandedSteps) {
            // Scale timeout with expected motion duration so high-ratio long moves do not
            // false-timeout. stepperX/Y MaxSpeed in firmware is 2000 steps/s; use a conservative
            // 800 steps/s effective rate plus a 10s base.
            const float effectiveStepsPerSecond = 800f;
            var seconds = 10.0 + (commandedSteps / effectiveStepsPerSecond);
            // Hard floor and ceiling for safety.
            seconds = Math.Max(10.0, Math.Min(seconds, 600.0));
